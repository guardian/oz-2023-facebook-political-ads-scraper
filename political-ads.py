...
def savePosts(resultsJson, pageID):
	print("Saving data for", data['page_name'])	
...
# 		print(result)
		...
		scraperwiki.sqlite.save(unique_keys=["ad_id"], data=data, table_name='ads_by_politicians')
...
pollies['page_id'].round(0)
pollies = pollies[pollies['gov_level'] == 'Federal']
page_ids = list(pollies['page_id'])
...
for x in range(upto, len(page_ids)):
	print("Upto", x, " of ", len(page_ids))
	getPosts(page_ids[x],initialSinceDate, fields)
